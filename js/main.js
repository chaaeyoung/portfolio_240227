...
html 요소의 속성을 변경하여 다양한 애니메이션 효과를 줌
   ================================================== */
    ...
    // 실행 중 동적으로 할당될 수 있으므로 초기값을 주지 않음

    // prev
    // current < 0 -> current 변수값이 0보다 작은지 확인
    // prev 함수 저의 - animate 함수 호출하여 이전 방향으로 애니메이션 수행
    ...
    $('.popup>button.next').on('click',function(){
        ...
        max = container.children().length;
        // pop 이미지 개수를 max 변수에 할당
        container.addClass('margin-left','-800px').append(container.children()[max - 1]);
        next(); // 함수호출
    });
    // .parent() - 부모요소 선택
